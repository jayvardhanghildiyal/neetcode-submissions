class Solution {
    public int rob(int[] nums) {
        ...
        arr1[0] = nums1[0];
        arr2[0] = nums2[0];
        arr1[1] = Math.max(nums1[1], nums1[0]);
        arr2[1] = Math.max(nums2[1], nums2[0]);

        for (int i = 2; i < nums1.length; i++) {
            arr1[i] = Math.max(nums1[i] + arr1[i - 2], arr1[i - 1]);
        }
        for (int i = 2; i < nums2.length; i++) {
            arr2[i] = Math.max(nums2[i] + arr2[i - 2], arr2[i - 1]);
        }

        return Math.max(arr1[nums.length - 2], arr2[nums.length - 2]);
    }
